...
pub fn inject_dynamic(lua_api: &mut LuaApi) {
    ...
    lua_api.add_dynamic_function("Net", "set_player_health", |api_ctx, lua_ctx, params| {
        let (player_id, health): (mlua::String, i32) = lua_ctx.unpack_multi(params)?;
        let player_id_str = player_id.to_str()?;
    ...
    lua_api.add_dynamic_function(
        ...
        |api_ctx, lua_ctx, params| {
            ...
            if let Some(player_data) = &net.get_player_data(player_id_str) {
                lua_ctx.pack_multi(player_data.base_health)
            } else {
                Err(create_player_error(player_id_str))
            }
        },
    ...
    lua_api.add_dynamic_function(
        "Net",
        "set_player_base_health",
        |api_ctx, lua_ctx, params| {
            let (player_id, base_health): (mlua::String, i32) = lua_ctx.unpack_multi(params)?;
            let player_id_str = player_id.to_str()?;
    ...
    lua_api.add_dynamic_function(
        ...
        |api_ctx, lua_ctx, params| {
            ...
            if let Some(player_data) = &net.get_player_data(player_id_str) {
                lua_ctx.pack_multi(player_data.max_health().max(0))
            } else {
                Err(create_player_error(player_id_str))
            }
        },
    ...
    lua_api.add_dynamic_function(
        ...
        |api_ctx, lua_ctx, params| {
            ...
            let (player_id, max_health): (mlua::String, i32) = lua_ctx.unpack_multi(params)?;
            let player_id_str = player_id.to_str()?;
    ...
    lua_api.add_dynamic_function("Net", "get_player_emotion", |api_ctx, lua_ctx, params| {
        ...
        if let Some(player_data) = &net.get_player_data(player_id_str) {
            lua_ctx.pack_multi(player_data.emotion.as_str())
        } else {
            Err(create_player_error(player_id_str))
        }
    });
    ...
    lua_api.add_dynamic_function("Net", "set_player_emotion", |api_ctx, lua_ctx, params| {
        let (player_id, emotion): (mlua::String, String) = lua_ctx.unpack_multi(params)?;
        let player_id_str = player_id.to_str()?;
