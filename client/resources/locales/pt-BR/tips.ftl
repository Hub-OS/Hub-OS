...
tip-change-display-name = Seu apelido pode ser alterado em Opções -> Online -> Trocar Apelido.
tip-view-display-name = Você pode ver os apelidos ao colocar o cursor do mouse em outro personagem no Cibermundo.
tip-updating-mods = Você deveria atualizar os mods de vez em quando em Opções -> Mods -> Atualizar Mods.
tip-server-music = Mods de Recursos podem customizar Visuais, Sons e Música, incluindo a musica padrão do servidor.
...
tip-special-button = { $binding } é usada para ativar o equipamento extra, como escudos. Isso é semelhante ao B+Esquerda de outros jogos.
tip-auto-emotes = Um emoji de pensamento vai aparecer sobre os jogadores mexendo nas configurações.
tip-move-cards-in-bulk = Edição de Pastas permite que você mova vários chips de uma vez ao apertar { $binding } por um segundo.
