    ...
    pub fn default_key_bindings(key_style: KeyStyle) -> HashMap<Input, Vec<Key>> {
        match key_style {
            KeyStyle::Mix => HashMap::from([
                ...
                (Input::Confirm, vec![Key::Space, Key::J, Key::X]),
                (Input::UseCard, vec![Key::Space, Key::J, Key::X]),
                (
                    Input::Cancel,
                    vec![Key::LShift, Key::K, Key::Escape, Key::Z],
                ),
                (Input::Shoot, vec![Key::LShift, Key::K, Key::Z]),
                (Input::Sprint, vec![Key::LShift, Key::K, Key::Z]),
                (Input::Map, vec![Key::M]),
            ...
            KeyStyle::Emulator => HashMap::from([
                ...
                (Input::Confirm, vec![Key::X]),
                (Input::UseCard, vec![Key::X]),
                (Input::Cancel, vec![Key::Z]),
                (Input::Shoot, vec![Key::Z]),
                (Input::Sprint, vec![Key::Z]),
                (Input::Map, vec![Key::M]),
                (Input::Option, vec![Key::Return]),
