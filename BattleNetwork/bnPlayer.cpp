...
void Player::ActivateFormAt(int index)
{
    ...
    if (index >= 0 || index < forms.size()) {
        ...
        //If we're activating a form, cancel the drag cooldown and allow us to move.
        SetDragFinishCooldown(frames(0));
        //Return to idle when taking a new form.
        animationComponent->SetAnimation(PLAYER_IDLE);
        //Remove stun and freeze.
        Stun(frames(0));
        IceFreeze(frames(0));
        //Cancel all actions
        ClearActionQueue();
