  ...
  void FillScreen(sf::Vector2u textureSize) {
    ...
    for (unsigned int i = 0; i < occuranceX; ++i) {
      for (unsigned int j = 0; j < occuranceY; ++j) {
        ...
        quad[1].position = sf::Vector2f((float)(i * textureSize.x * 2), (float)((j + 1) * textureSize.y * 2));
        quad[2].position = sf::Vector2f((float)((i + 1) * textureSize.x * 2), (float)(j * textureSize.y * 2));
        quad[3].position = sf::Vector2f((float)((i + 1) * textureSize.x * 2), (float)((j + 1) * textureSize.y * 2));
