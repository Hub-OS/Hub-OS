...
public:
  PerlinNoise() {
    // Initialize the p vector with the reference values
    p = {
  ...
  PerlinNoise(unsigned int seed) {
    ...
    std::is_permutation(p.begin(), p.end(), 0);
...
private:
  ...
  double Gradient(int hash, double x, double y, double z) {
    int h = hash & 15;
    // Convert lower 4 bits of hash into 12 Gradientient directions
    double u = h < 8 ? x : y,
