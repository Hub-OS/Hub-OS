...
CubeCardAction::CubeCardAction(std::shared_ptr<Character> actor) : 
  CardAction(actor, "PLAYER_IDLE"){
  this->SetLockout({ CardAction::LockoutType::sequence });
}

void CubeCardAction::OnExecute(std::shared_ptr<Character> user) {
  auto actor = this->GetActor();
  
  auto cube = std::make_shared<Cube>(actor->GetField());
